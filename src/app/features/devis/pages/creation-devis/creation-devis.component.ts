...
export class CreationDevisComponent implements OnInit {
  ...
  async generatePDF() {
    ...
    img.onload = () => {
      ...
      doc.setFont('helvetica', 'bold');
      doc.setFont('helvetica', 'normal');
      doc.text(`Date: ${this.formatDate(this.devis.date)}`, margin, y); y += 5;

      // Informations client
      doc.setFont('helvetica', 'bold');
      doc.text('Facturé à:', pageWidth - margin - 80, y - 10);
      doc.setFont('helvetica', 'normal');
      doc.text(this.devis.nom, pageWidth - margin - 80, y - 5);
      doc.text(this.devis.adresse, pageWidth - margin - 80, y);
      doc.text(`Permis: ${this.devis.permis}`, pageWidth - margin - 80, y + 5);
      ...
      doc.setFont('helvetica', 'normal');
      doc.text(`Date: ${this.devis.dateStage}`, margin, y); y += 5;
      doc.text(`Horaires: ${this.devis.horaires}`, margin, y); y += 5;
      doc.text(`Lieu: ${this.devis.lieu}`, margin, y); y += 15;
      ...
      doc.setFont('helvetica', 'normal');
      doc.text(this.devis.modePaiement, margin + 40, y);
      ...
      // Sauvegarde
      doc.save(`devis-${this.entreprise.nom}-${this.devis.nom}.pdf`);
    };
  }
}
